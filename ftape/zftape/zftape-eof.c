 ...
 *      
 *      Modified for zftape 1996-1998 Claus Heine.
...
static const char linux_tape_label[] = "Linux raw format V";
enum { 
...
int zft_ftape_validate_label(zftape_info_t *zftape, char *label)
{
	static char tmp_label[FT_LABEL_SZ + 1];
	int result = 0;
	TRACE_FUN(ft_t_any);
	...
	zftape->ftape_fmt_version = 0;
	if (memcmp(label, linux_tape_label, strlen(linux_tape_label)) == 0) {
		int pos = strlen(linux_tape_label);
		while (label[pos] >= '0' && label[pos] <= '9') {
			zftape->ftape_fmt_version *= 10;
			zftape->ftape_fmt_version = label[ pos++] - '0';
		}
		result = (zftape->ftape_fmt_version >= min_fmt_version &&
			  zftape->ftape_fmt_version <= max_fmt_version);
	}
	TRACE(ft_t_noise, "format version = %d", zftape->ftape_fmt_version);
	TRACE_EXIT result;
}
...
void zft_ftape_extract_file_marks(zftape_info_t *zftape, __u8* address)
{
	int i;
	TRACE_FUN(ft_t_any);
	
	zftape->eof_map = NULL;
	if (zftape->ftape->format_code == fmt_var ||
	    zftape->ftape->format_code == fmt_big) {
		__u8* end;
		__u8* start = ftape_find_end_of_bsm_list(address);

		zftape->nr_eof_marks = 0;
		if (start) {
			start += 3; /* skip end of list mark */
			end = find_end_of_eof_list(start, 
						   address + FT_SEGMENT_SIZE);
			if (end && end - start <= FT_FSL_SIZE) {
				zftape->nr_eof_marks = ((end - start) / 
						    sizeof(eof_mark_union));
				zftape->eof_map = (eof_mark_union *)start;
			} else {
				TRACE(ft_t_err,
		...
		zftape->eof_map = (eof_mark_union *)&address[FT_FSL];
		zftape->nr_eof_marks = GET2(address, FT_FSL_CNT);
	}
	TRACE(ft_t_noise, "number of file marks: %d", zftape->nr_eof_marks);
	if (zftape->ftape_fmt_version == 1) {
		TRACE(ft_t_info, "swapping version 1 fields");
		/* version 1 format uses swapped sector and segment
		 * fields, correct that !  
		 */
		for (i = 0; i < zftape->nr_eof_marks; ++i) {
			__u16 tmp = GET2(&zftape->eof_map[i].mark.segment,0);
			PUT2(&zftape->eof_map[i].mark.segment, 0, 
			     GET2(&zftape->eof_map[i].mark.date,0));
			PUT2(&zftape->eof_map[i].mark.date, 0, tmp);
		}
	}
	for (i = 0; i < zftape->nr_eof_marks; ++i) {
		TRACE(ft_t_noise, "eof mark: %5d/%2d",
		      GET2(&zftape->eof_map[i].mark.segment, 0), 
		      GET2(&zftape->eof_map[i].mark.date,0));
	}
	TRACE_EXIT;
}

void zft_clear_ftape_file_marks(zftape_info_t *zftape)
{
	TRACE_FUN(ft_t_flow);
	...
	memset(zftape->eof_map, 0, zftape->nr_eof_marks * sizeof(__u32));
	zftape->nr_eof_marks = 0;
	PUT2(zftape->hseg_buf, FT_FSL_CNT, 0); /* nr of eof-marks */
	zftape->header_changed = 1;
	zft_update_label(zftape, zftape->hseg_buf);
	TRACE_EXIT;
}
