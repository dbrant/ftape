 ...
 *  is done. In raw ECC mode this includes the ECC sectors, so that a
 *  segment without bad sectors accounts for the full 32kb that were
 *  read from the tape.
 */
			    ...
			    unsigned int segment)
{
	...
	if (size > 0 && zftape->raw_ecc) {
		size += FT_ECC_SECTORS * FT_SECTOR_SIZE;
	}
	return size;
}

/*  Decide whether the ECC sectors are handed out to user space and
 ...
 *  tape cannot be written to during this session: the ECC sectors are
 *  generated by the driver (or the FDC) when writing, so a segment
 *  image that carries its ECC data can be read, but not written back.
 */
void zft_update_raw_ecc(zftape_info_t *zftape)
{
	unsigned int raw_ecc = (!zftape->qic_mode && zftape->write_protected);
	TRACE_FUN(ft_t_flow);

	if (raw_ecc == zftape->raw_ecc) {
		TRACE_EXIT;
	}
	TRACE(ft_t_info, "%s ECC sectors %s the raw data stream",
	      raw_ecc ? "including" : "excluding",
	      raw_ecc ? "in" : "from");
	zftape->raw_ecc = raw_ecc;
	if (zftape->header_read) {
		/*  the segment size just changed under our feet, so
		 *  all cached positions and sizes are stale now.
		 */
