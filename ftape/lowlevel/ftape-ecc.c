...
static inline unsigned long gfmul_exp_long(unsigned long a, int b)
{
	...
	if (sizeof(long) == 4) {
		...
		printk("Error: size of long is %d bytes.\n", (int)sizeof(long));
		return -1;
	}
}
			 ...
			 Matrix Ainv)
{
	...
	if (!det) {
		printk("Inversion failed (3 CRC errors, >0 CRC failures)\n");
		return 0;
	}
...
static inline int gfinv2(__u8 l0, __u8 l1, Matrix Ainv)
{
	...
	det = gfadd(t1, t2);
	if (!det) {
		printk("Inversion failed (2 CRC errors, >0 CRC failures)\n");
		return 0;
	}
	log_det = 255 - gflog[det];
			      ...
			      int stride)
{
	...
	while (data < end) {
		...
		if (sizeof(long) == 4) {
			...
			printk("Error: long is of size %d.\n",
			       (int) sizeof(long));
			return;
		}
		p0 = gfadd_long(t2, p1);
				...
				SectorMap * correction_map)
{
	...
	case 0:
		...
		if (s[0] == 0) {
			/* more than one error */
			printk("ECC failed (0 CRC errors, >1 CRC failures)\n");
			return -1;
		}
		t1 = gfdiv(s[1], s[0]);
		if ((bad_loc[nbad++] = gflog[t1]) >= nblocks) {
			printk("ECC failed (0 CRC errors, >1 CRC failures)\n");
			printk("attempt to correct data at %d.\n", bad_loc[0]);
			return -1;
		}
		error_mag[0] = s[1];
		break;
	case 1:
		...
		if (t1 == 0 && t2 == 0) {
			...
			printk("ECC failed (1 erasure, >1 error).\n");
			return -1;
		} else {
			/* one erasure, one error: */
			if ((bad_loc[nbad++] = gflog[gfdiv(t1, t2)]) 
			    >= nblocks) {
				printk("ECC failed (1 CRC errors, >1 CRC failures).\n");
				printk("attempt to correct data at %d.\n", bad_loc[1]);
				return -1;
			}
	...
	default:
		printk("Internal Error: number of CRC errors > 3.\n");
		return -1;
	}
	...
	if (c0 || c1 || c2) {
		printk("ECC self-check failed, too many errors.\n");
		return -1;
	}
	return ncorrected;
}
...
static int sanity_check(unsigned long *data, int nblocks)
{
	...
	if (!compute_syndromes(data, nblocks, s)) {
		printk("Internal Error: syndrome self-check failed.\n");
		return 0;
	}
	return 1;
}
...
int ftape_ecc_correct_data(struct memory_segment *mseg)
{
		...
		for (i = 0; i < mseg->blocks; i++) {
			if (BAD_CHECK(mseg->read_bad, i)) {
				if (nerasures >= 3) {
					...
					printk("ECC failed (>3 CRC errors).\n");
					return ECC_FAILED;
				}	/* if */
	...
	if (ncorrected && nerasures == 0) {
		printk("block contained error not caught by CRC.\n");
	}
#if 0
