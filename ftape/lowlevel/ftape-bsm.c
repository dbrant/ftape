...
void fix_tape(__u8 * buffer, ft_format_type new_code)
{
	...
	__u8 *dst_ptr = ftape->bad_sector_map;
	SectorMap map;
	unsigned int sector = 1;
	int i;

	if (format_code != fmt_var && format_code != fmt_big) {
		memcpy(list, ftape->bad_sector_map, sizeof(list));
		memset(ftape->bad_sector_map, 0, sizeof(ftape->bad_sector_map));
...
static void print_bad_sector_map(ftape_info_t *ftape)
{
	...
	if (ftape->format_code == fmt_big || 
	    ftape->format_code == fmt_var || 
	    ftape->format_code == fmt_1100ft) {
		SectorCount *ptr = (SectorCount *)ftape->bad_sector_map;
		unsigned int sector;
		...
		while((sector = get_sector(ptr++)) != 0) {
			if ((ftape->format_code == fmt_big || 
				...
				TRACE(ft_t_flow, "bad segment at sector: %6d",
				      sector & 0x7fffff);
			} else {
				++total_bad;
				TRACE(ft_t_flow, "bad sector: %6d", sector);
			}
		}
		...
		for (i = ftape->first_data_segment;
			...
			SectorMap map = ((SectorMap *) ftape->bad_sector_map)[i];
	...
	good_sectors =
		((ftape->segments_per_track * ftape->tracks_per_tape - ftape->first_data_segment)
...
void ftape_extract_bad_sector_map(ftape_info_t *ftape, __u8 * buffer)
{
	TRACE_FUN(ft_t_any);
	...
	if (ftape->format_code == fmt_var || ftape->format_code == fmt_big) {
		 ...
		 */
		ftape->bad_sector_map = &buffer[FT_HEADER_END];
	} else {
		/* non-wide QIC-80 tapes have a failed sector log area that
		 * mustn't be included in the bad sector map.
		 */
		ftape->bad_sector_map = &buffer[FT_FSL + FT_FSL_SIZE];
	}
	if (ftape->format_code == fmt_1100ft || 
	    ftape->format_code == fmt_var    ||
	    ftape->format_code == fmt_big) {
		ftape->bsm_hash_ptr = (SectorCount *)ftape->bad_sector_map;
	} else {
		ftape->bsm_hash_ptr = NULL;
	}
	bsm_debug_fake();
	if (TRACE_LEVEL >= ft_t_info) {
		print_bad_sector_map(ftape);
	}
	TRACE_EXIT;
}
...
static int forward_seek_entry(ftape_info_t *ftape,
			      int segment_id, 
			      SectorCount **ptr, 
			      SectorMap *map)
{
...
static int backwards_seek_entry(ftape_info_t *ftape,
				int segment_id,
				SectorCount **ptr,
				SectorMap *map)
{
	...
	if (*ptr <= (SectorCount *)ftape->bad_sector_map) {
		*map = 0;
		return 0;
	}
	do {
		sector  = get_sector(--(*ptr));
		segment = cvt2segment(sector);
	} while (*ptr > (SectorCount *)ftape->bad_sector_map && segment > segment_id);
	if (segment > segment_id) { /*  at start of list, no entry found */
		...
		*map = 0;
		return 0;
		   ...
		   (ftape->format_code == fmt_big || ftape->format_code == fmt_var)) {
		*map = EMPTY_SEGMENT;
		return FT_SECTORS_PER_SEGMENT;
		...
		*map = cvt2map(sector);
		while(*ptr > (SectorCount *)ftape->bad_sector_map) {
			sector = get_sector(--(*ptr));
			segment = cvt2segment(sector);
...
void ftape_put_bad_sector_entry(ftape_info_t *ftape,
				int segment_id, SectorMap new_map)
{
	SectorCount *ptr = (SectorCount *)ftape->bad_sector_map;
	int count;
	int new_count;
	...
	/* correct the cached bsm entries */
	ftape->last_reference = segment_id;
	ftape->map = new_map;

	if (ftape->format_code == fmt_1100ft || 
	    ftape->format_code == fmt_var || 
	    ftape->format_code == fmt_big) {
		count = forward_seek_entry(ftape, segment_id, &ptr, &map);
		ftape->bsm_hash_ptr = (SectorCount *)ptr;
		...
		if (count != new_count) {
			...
			memmove(ptr + new_count, ptr + count,
				(size_t)(hi_ptr - (ptr + count)) *
				sizeof(SectorCount));			      
		}
		TRACE(ft_t_info, "putting map 0x%08x at %p, segment %d",
		...
		((SectorMap *) ftape->bad_sector_map)[segment_id] = new_map;
	}
	TRACE_EXIT;
}

SectorMap ftape_get_bad_sector_entry(ftape_info_t *ftape, int segment_id)
{
	if (ftape->used_header_segment == -1) {
		/*  When reading header segment we'll need a blank map.
		 */
		return 0;
	} else if (ftape->bsm_hash_ptr != NULL) {
		/*  Invariants:
		...
		if (segment_id > ftape->last_reference) {
			/*  Skip all sectors before segment_id
			 */
			forward_seek_entry(ftape, segment_id, &ftape->bsm_hash_ptr, &ftape->map);
		} else if (segment_id < ftape->last_reference) {
			/* Skip backwards until begin of buffer or
			 * first sector in segment_id 
			 */
			backwards_seek_entry(ftape, segment_id, &ftape->bsm_hash_ptr, &ftape->map);
		}		/* segment_id == last_reference : keep map */
		ftape->last_reference = segment_id;
		return ftape->map;
	} else {
		return ((SectorMap *) ftape->bad_sector_map)[segment_id];
	}
}
 ...
 */
void ftape_init_bsm(ftape_info_t *ftape)
{
	ftape->bad_sector_map = NULL;
	ftape->bsm_hash_ptr   = NULL;
